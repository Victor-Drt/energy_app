...
class LineChartTensaoHora extends StatelessWidget {
  final List<TensaoHora> itensTensaoHora;

  LineChartTensaoHora({required this.itensTensaoHora});
  ...
  List<FlSpot> _generateSpots() {
    ...
    if (itensTensaoHora.isEmpty || itensTensaoHora.first.registros == null) {
      return [];
    }

    // Extraindo registros do primeiro item da lista de `ConsumoHora`
    final registros = itensTensaoHora.first.registros!;
    return registros
        .where((registro) => registro.hora != null && registro.tensaoMedia != null)
        .map((registro) {
          final hora = int.tryParse(registro.hora!) ?? 0;
          final potencia = registro.tensaoMedia!.toStringAsFixed(4);
  ...
  double _getMinY() {
    if (itensTensaoHora.isEmpty || itensTensaoHora.first.registros == null) return 0;

    return itensTensaoHora.first.registros!
        .where((registro) => registro.tensaoMedia != null)
        .map((registro) => registro.tensaoMedia!)
        .reduce((a, b) => a < b ? a : b);
  }
